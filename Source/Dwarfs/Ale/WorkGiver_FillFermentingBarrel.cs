...
namespace TheEndTimes_Dwarfs
{
    public class WorkGiver_FillFermentingBarrel : WorkGiver_Scanner
    {
        ...
        public static void Reset()
        {
            ...
            NoWortTrans = "RH_TET_Dwarfs_NoWortAle".Translate();
        }
        ...
        {
            var barrel = (Building_FermentingAleBarrel)t;
            var t2 = FindWort(pawn, barrel);
            return new Job(DefDatabase<JobDef>.GetNamed("RH_TET_Dwarfs_FillFermentingBarrel"), t, t2);
        }

        private Thing FindWort(Pawn pawn, Building_FermentingAleBarrel barrel)
        {
            bool Predicate(Thing x) => !x.IsForbidden(pawn) && pawn.CanReserve(x, 1, -1, null, false);
            var position = pawn.Position;
            var map = pawn.Map;
            var thingReq = ThingRequest.ForDef(ThingDef.Named("RH_TET_Dwarf_AleWort"));
