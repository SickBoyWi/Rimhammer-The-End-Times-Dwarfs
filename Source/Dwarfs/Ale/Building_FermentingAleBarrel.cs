...
namespace TheEndTimes_Dwarfs
{
    ...
    public class Building_FermentingAleBarrel : Building
    {
        ...
        private Material BarFilledMat
        {
            get
            {
                ...
                {
                    this.barFilledCachedMat = SolidColorMaterials.SimpleSolidColorMaterial(Color.Lerp(Building_FermentingAleBarrel.BarZeroProgressColor, Building_FermentingAleBarrel.BarFermentedColor, this.Progress), false);
                }
        ...
        public void AddWort(int count)
        {
            ...
            if (this.Fermented)
            {
                Log.Warning("Tried to add wort to a barrel that is already fully fermented.");
                return;
            }
        ...
        public override string GetInspectString()
        {
            ...
            if (!this.Empty && !comp.Ruined)
            {
                if (this.Fermented)
                {
                    stringBuilder.AppendLine("RH_TET_Dwarfs_ContainsAle".Translate(new object[]
                    {
                        this.wortCount,
                        20
                    }));
                ...
                {
                    stringBuilder.AppendLine("RH_TET_Dwarfs_ContainsWortAle".Translate(new object[]
                    {
                        this.wortCount,
                        20
                    }));
        ...
        public Thing TakeOutAle()
        {
            if (!this.Fermented)
            {
                Log.Warning("Tried to get ale but it's not yet fermented.");
                return null;
            }
            Thing thing = ThingMaker.MakeThing(ThingDef.Named("RH_TET_Dwarf_Ale"), null);
        ...
        protected override void DrawAt(Vector3 drawLoc, bool flip = false)
        {
            ...
            if (!this.Empty)
            {
                ...
                {
                    center = drawPos,
                    size = Building_FermentingAleBarrel.BarSize,
                    fillPercent = (float)this.wortCount / 20f,
                    filledMat = this.BarFilledMat,
                    unfilledMat = Building_FermentingAleBarrel.BarUnfilledMat,
                    margin = 0.1f,
                    rotation = Rot4.North
                });
            }
