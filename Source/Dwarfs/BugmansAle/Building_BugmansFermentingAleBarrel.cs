...
namespace TheEndTimes_Dwarfs
{
    ...
    public class Building_BugmansFermentingAleBarrel : Building
    {
        private int wortCount;

        private string wortType;
        ...
        private Material BarFilledMat
        {
            get
            {
                ...
                {
                    this.barFilledCachedMat = SolidColorMaterials.SimpleSolidColorMaterial(Color.Lerp(Building_BugmansFermentingAleBarrel.BarZeroProgressColor, Building_BugmansFermentingAleBarrel.BarFermentedColor, this.Progress), false);
                }
        ...
        public void AddWort(int count)
        {
            ...
            if (this.Fermented)
            {
                Log.Warning("Tried to add Bugman's wort to a barrel that is already fully fermented.");
                return;
            }
        ...
        public override string GetInspectString()
        {
            ...
            if (!this.Empty && !comp.Ruined)
            {
                if (this.Fermented)
                {
                    stringBuilder.AppendLine("RH_TET_Dwarfs_ContainsBugmansAle".Translate(new object[]
                    {
                        this.wortCount,
                        20
                    }));
                ...
                {
                    stringBuilder.AppendLine("RH_TET_Dwarfs_ContainsBugmansWortAle".Translate(new object[]
                    {
                        this.wortCount,
                        20
                    }));
        ...
        public Thing TakeOutAle()
        {
            if (!this.Fermented)
            {
                Log.Warning("Tried to get Bugman's ale but it's not yet fermented.");
                return null;
            }
            Thing thing = ThingMaker.MakeThing(ThingDef.Named("RH_TET_Dwarf_BugmansXXX"), null);
        ...
        protected override void DrawAt(Vector3 drawLoc, bool flip = false)
        {
            ...
            if (!this.Empty)
            {
                ...
                {
                    center = drawPos,
                    size = Building_BugmansFermentingAleBarrel.BarSize,
                    fillPercent = (float)this.wortCount / 20f,
                    filledMat = this.BarFilledMat,
                    unfilledMat = Building_BugmansFermentingAleBarrel.BarUnfilledMat,
                    margin = 0.1f,
                    rotation = Rot4.North
                });
            }
