...
namespace TheEndTimes_Dwarfs
{
    partial class HarmonyPatches_OtherStuff
    {
        ...
        // Show the pawn's hair when they're wearing a slayer head tattoo.
        [HarmonyPatch(typeof(PawnRenderTree))]
        [HarmonyPatch("AdjustParms")]
        internal static class PatchPawnRenderTree_AdjustParms
        {
            private static void Postfix(Verse.PawnRenderTree __instance, ref Verse.PawnDrawParms parms)
            {
                var pawn = __instance.pawn;

                if (!DwarfsUtil.IsSlayer(pawn))
                    return;

                bool hasSlayerHeadTattoo = false;
                bool hasOtherHeadGear = false;

                foreach (Apparel item in pawn.apparel.WornApparel)
                {
                    if (item != null && item.def.defName.Contains("Slayer_Tattoo_Head"))
                    {
                        hasSlayerHeadTattoo = true;
                    }
                    else if (item.def.apparel.bodyPartGroups.Contains(RimWorld.BodyPartGroupDefOf.UpperHead) ||
                        item.def.apparel.bodyPartGroups.Contains(RimWorld.BodyPartGroupDefOf.FullHead))
                    {
                        hasOtherHeadGear = true;
                    }
                }

                if (hasSlayerHeadTattoo && !hasOtherHeadGear)
                {
        ...
        // Don't show slayer tattoos for removal in the pawn gear tab.
        /**  Decided against this. This does work. We decided to let them show and be removed so the player has all the info they may need in an easily accessible manner.
        [HarmonyPatch(typeof(ITab_Pawn_Gear))]
        [HarmonyPatch("DrawThingRow")]
        internal static class Patch_ITab_Pawn_Gear_DrawThingRow
        {
            private static bool Prefix(ITab_Pawn_Gear __instance, ref float y, float width, Thing thing, bool inventory = false)
            {
                if (thing != null && thing.def != null && thing.def.defName.Contains("Slayer_Tattoo"))
                    return false;
                else
                    return true;
            }
